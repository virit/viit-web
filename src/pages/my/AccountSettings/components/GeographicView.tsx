...
@connect(
  ({
    ...
    const { province, city } = myAndaccountsettings;
    return {
      province,
      city,
      loading: loading.models.myAndaccountsettings,
    };
  },
)
class GeographicView extends Component<GeographicViewProps> {
  componentDidMount = () => {
    ...
    if (dispatch) {
      // dispatch({
      //   type: 'myAndaccountsettings/fetchProvince',
      // });
    }
  };
  ...
  componentDidUpdate(props: GeographicViewProps) {
    ...
    if (!props.value && !!value && !!value.province) {
      if (dispatch) {
        dispatch({
          type: 'myAndaccountsettings/fetchCity',
